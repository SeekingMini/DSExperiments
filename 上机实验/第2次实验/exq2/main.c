...
void difference(SeqList*,SeqList*);

/*
 * 值得注意的是，题目中写着没有特别指明同一表中的元素值各不相同，所以可能出现多重集
 */
int main() {
    ElemType e;
    SeqList A;
    ...
    C.elem = (ElemType*)malloc(MAXSIZE* sizeof(ElemType));
...
/*
 * 求A与B的交集
 * 算法思想：首先对A中所有元素进行遍历，如果这个元素在B中没有，就删除这个元素，最终得到的C就是A与B的交集。时间复杂度为O(n^2)。
 */
